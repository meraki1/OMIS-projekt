...
Public Class Prijevoznici
    ...
    Private Sub BindFormViewData()
        ...
        Using conn As New SqlConnection(constr)
            Dim cmd As New SqlCommand("CRUDFormPrijevoznik", conn)
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs)
        ...
        SearchPrijevoznikForButton(searchValue)
    End Sub

    <System.Web.Services.WebMethod()>
    Public Shared Function SearchPrijevoznik(ByVal prefixText As String) As String()
        Dim suggestions As New List(Of String)
        Dim constr As String = ConfigurationManager.ConnectionStrings("MyConnectionString").ConnectionString
        Using conn As New SqlConnection(constr)
            Dim cmd As New SqlCommand("SELECT Naziv FROM Prijevoznik WHERE Naziv LIKE '%' + @prefixText + '%'", conn)
    ...
    Protected Sub SearchPrijevoznikForButton(ByVal searchValue As String)
        Dim constr As String = ConfigurationManager.ConnectionStrings("MyConnectionString").ConnectionString
        Using conn As New SqlConnection(constr)
            Dim cmd As New SqlCommand("SELECT * FROM Prijevoznik WHERE Naziv LIKE '%' + @searchValue + '%'", conn)
    ...
    Protected Sub FormView1_ItemUpdating(sender As Object, e As FormViewUpdateEventArgs)
        Dim IDPrijevoznik As Integer = Integer.Parse(DirectCast(FormView1.FindControl("IDPrijevoznikHiddenField"), HiddenField).Value)
        ...
        Using conn As New SqlConnection(constr)
            ...
            cmd.Parameters.AddWithValue("@IDPrijevoznik", IDPrijevoznik)
